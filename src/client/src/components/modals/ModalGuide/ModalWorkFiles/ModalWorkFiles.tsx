...
import {ModalWorkFilesProps} from "@/components/modals/ModalGuide/ModalWorkFiles/types";
import styles from './ModalWorkFiles.module.scss';

const DOWNLOAD_ALL_ID = 0;

const ModalWorkFiles = (props: ModalWorkFilesProps) => {
    ...
    const handleDownloadAll = async () => {
        ...
        try {
            await downloadWorkFilesArchive(workId, workName);

            addSnackbar({
                type: 'success',
                text: `Скачивание архива из "${workName}" запущено`
            });
        } catch (error) {
            addSnackbar({
                type: 'error',
    ...
    const handleDownload = async (fileId: number, fileName: string) => {
        ...
        try {
            await downloadWorkFile(fileId, fileName);
        } catch (error) {
            addSnackbar({
                type: 'error',
    ...
    return (
        ...
        >
            {files.length === 0 ? (
                ...
                >
                    <Text>У этой работы нет загруженных файлов</Text>
                </Placeholder>
            ) : (
