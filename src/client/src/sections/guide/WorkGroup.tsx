...
type WorkGroupFilterState = {
    operationGroupId: number;
    operationId: number;
};

const initialFilters: WorkGroupFilterState = {
    operationGroupId: 0,
    operationId: 0,
};
...
const WorkGroup = (
    ...
    const operationGroupSelectFilter = useSelectFilter();
    const operationSelectFilter = useSelectFilter();
    ...
    const [filters, setFilters] = useState<WorkGroupFilterState>(initialFilters);
    ...
    const [modals, setModals] = useState<OpenModalsType<
        'modal-manage-work-group' | 'modal-remove-work-group' | 'modal-multi-remove-work-group'
    ...
    const { createController: createOperationGroupController } = useController([]);
    const { createController: createOperationController } = useController([filters.operationGroupId]);
    ...
    useEffect(() => {
        const controller = createOperationGroupController();
    ...
    useEffect(() => {
        ...
        ApiService.guide.operation.get({
            controller,
            errorOptions: {show: false},
            options: filters.operationGroupId
                ? {operationGroupId: filters.operationGroupId}
                : undefined,
        }).then(({status, data}) => {
            if (status === 'success') {
                ...
                if (filters.operationId && !data.some(({id}) => id === filters.operationId)) {
                    mergeState({operationId: 0}, setFilters);
                }
            }
        });
    }, [filters.operationGroupId]);
    ...
    const getData = async () => {
        ...
        const filterOptions: GetWorkGroupsTableFilters = {
            ...(filters.operationGroupId ? {operationGroupId: filters.operationGroupId} : {}),
    ...
    }, [tableOptions.sorting, tableOptions.search, tableOptions.page, tableOptions.rows, filters.operationGroupId, filters.operationId]);
    ...
    const operationGroupOptions = useMemo<CustomSelectOptionInterface[]>(
        () => [{value: 0, label: 'Все группы операций'}, ...operationGroups],
        [operationGroups]
    );
    const operationOptions = useMemo<CustomSelectOptionInterface[]>(
        () => [{value: 0, label: 'Все операции'}, ...operations],
        [operations]
    );
                    ...
                    <div className={styles.controls}>
                        <div className={styles.filters}>
                            <div className={styles.filter}>
                                <Select
                                    filterFn={operationGroupSelectFilter.filterFn}
                                    options={operationGroupOptions}
                                    searchable={true}
                                    disabled={loading.page || tableManage.editMode}
                                    value={filters.operationGroupId}
                                    onChange={(e) => {
                                        const operationGroupId = Number(e.target.value);
                                    ...
                                    onInputChange={operationGroupSelectFilter.onInputChange}
                                    onOpen={operationGroupSelectFilter.onOpen}
                                    onClose={operationGroupSelectFilter.onClose}
                                />
                            </div>
                            <div className={styles.filter}>
                                <Select
                                    filterFn={operationSelectFilter.filterFn}
                                    options={operationOptions}
                                    searchable={true}
                                    disabled={loading.page || tableManage.editMode}
                                    value={filters.operationId}
                                    onChange={(e) => {
                                        mergeState({operationId: Number(e.target.value)}, setFilters);
                                        mergeState({page: 0}, setTableOptions);
                                    }}
                                    onInputChange={operationSelectFilter.onInputChange}
                                    onOpen={operationSelectFilter.onOpen}
                                    onClose={operationSelectFilter.onClose}
                                />
                            </div>
                        </div>
                        <Search
