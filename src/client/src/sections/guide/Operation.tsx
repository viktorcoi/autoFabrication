...
const Operation = (
    ...
    const handleTableSave = async (changes: PatchOperationTableOptions) => {
        ...
        await ApiService.guide.operation.table.patch({
            options: changes,
            ...
            if (status === 'success') {
                ...
                if (data.error.length) {
                    snackbar.onActionClick = () => {
                        showErrors.open(table.data.filter(({id}) => ids.includes(String(id))).map((operation) => ({
                            id: operation.id,
                            name: operation.name
                        })), data);
                    }
    ...
    const handleRemove = () => {
        if (selected.length === 1) {
            const operation = table.data.find(({id}) => id === selected[0]);
            if (operation) {
                setModals({
                    id: 'modal-remove-operation',
                    show: true,
                    data: {
                        id: operation.id,
                        name: operation.name
            ...
            const operations = table.data.filter(({id}) => selected.includes(id));
            setModals({
                id: 'modal-multi-remove-operation',
                show: true,
                data: operations.map(({id, name}) => ({id, name}))
            })
        }
    };
    ...
    const onEventTable = (e: TableEvent) => {
        ...
        if (e.type === 'cellDoubleClick') {
            ...
            setModals({id: 'modal-manage-operation', show: true, data: e.row.id});
        }
        if (e.type === 'selected') {
        ...
        if (e.type === 'download') {
            const row = e.row as OperationTableRow;
            ...
            mergeState({actionSheet:
                <>
                    ...
                    >
                        {access.editing && (
                            <ActionSheetItem
                                onClick={() => setModals({id: 'modal-manage-operation', show: true, data: e.row.id})}
                        ...
                        {access.removing && (
                            <ActionSheetItem
                                onClick={() => setModals({id: 'modal-remove-operation', show: true, data: {
                                        id: e.row.id,
                                        name: e.row.name
                                    }})}
                                mode={'destructive'}
