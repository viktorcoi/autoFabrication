...
import {GetMaterialsTableFilters, MaterialTableRow, PatchMaterialTableOptions} from "@/apiService/apiGuide/types";
...
import ModalManageMaterial from "@/components/modals/ModalGuide/ModalManageMaterial/ModalManageMaterial";
import ModalFiltersMaterial from "@/components/modals/ModalFilters/ModalFiltersMaterial/ModalFiltersMaterial";
...
const Material = (
    ...
    const [tableFilters, setTableFilters] = useStoredFilters<GetMaterialsTableFilters>('material', {
        materialGroupId: 0,
    });
    const [modals, setModals] = useState<OpenModalsType<
        'modal-manage-material' | 'modal-remove-material' | 'modal-multi-remove-material' | 'modal-filters-material'
    ...
    const getData = async () => {
        ...
        await ApiService.guide.material.table.get({
    ...
    const handleTableSave = async (changes: PatchMaterialTableOptions) => {
        ...
        await ApiService.guide.material.table.patch({
            options: changes,
            ...
            if (status === 'success') {
                ...
                if (data.error.length) {
                    snackbar.onActionClick = () => {
                        showErrors.open(table.data.filter(({id}) => ids.includes(String(id))).map((material) => ({
                            id: material.id,
                            name: material.name
                        })), data);
                    }
    ...
    const handleRemove = () => {
        if (selected.length === 1) {
            const material = table.data.find(({id}) => id === selected[0]);
            if (material) {
                setModals({
                    id: 'modal-remove-material',
                    show: true,
                    data: {
                        id: material.id,
                        name: material.name
            ...
            const materials = table.data.filter(({id}) => selected.includes(id));
            setModals({
                id: 'modal-multi-remove-material',
                show: true,
                data: materials.map(({id, name}) => ({id, name}))
            })
        }
    };
    ...
    const onEventTable = (e: TableEvent) => {
        ...
        if (e.type === 'cellDoubleClick') {
            ...
            setModals({id: 'modal-manage-material', show: true, data: e.row.id});
        }
        if (e.type === 'selected') {
            ...
            mergeState({actionSheet:
                <>
                    ...
                    >
                        {access.editing && (
                            <ActionSheetItem
                                onClick={() => setModals({id: 'modal-manage-material', show: true, data: e.row.id})}
                        ...
                        {access.removing && (
                            <ActionSheetItem
                                onClick={() => setModals({id: 'modal-remove-material', show: true, data: {
                                        id: e.row.id,
                                        name: e.row.name
                                    }})}
                                mode={'destructive'}
    ...
    return (
        <>
            ...
            {'modal-filters-material' === modals.id ? (
                <ModalFiltersMaterial
                ...
                />
            ) : 'modal-multi-remove-material' === modals.id ? (
                <ModalMultiRemove
                    data={modals.data}
                    url={'/material'}
                    onClose={closeModal}
